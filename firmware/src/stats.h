...
inline void statsSave() {
  // NO-OP at runtime: on this board the RGB DMA crashes ("Cache disabled...")
  // on any flash/NVS write while the display is live. Stats are kept in RAM.
  return;
  if (!_dirty) return;
...
inline void petNameSet(const char* name) {
  // RAM-only — see statsSave() note (no flash writes while the RGB DMA runs).
